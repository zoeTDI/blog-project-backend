...
public class InfraConfigServiceImpl implements InfraConfigService, CommandLineRunner {
    ...
    public void initLocalCache() {
        ...
        LambdaQueryWrapper<InfraConfigPO> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.eq(InfraConfigPO::getDeleted, false);
        List<InfraConfigPO> configList = infraConfigMapper.selectList(queryWrapper);

        // 2. 转换为本地缓存映射（双稳态：清理旧缓存，装填新缓存）
        configCache.clear();
        for (InfraConfigPO config : configList) {
    ...
    public String getConfigValueByKey(String configKey) {
        ...
        return configCache.computeIfAbsent(configKey, key -> {
            ...
            LambdaQueryWrapper<InfraConfigPO> queryWrapper = new LambdaQueryWrapper<>();
            queryWrapper.eq(InfraConfigPO::getConfigKey, key)
                    .eq(InfraConfigPO::getDeleted, false)
                    .last("LIMIT 1");
            InfraConfigPO config = infraConfigMapper.selectOne(queryWrapper);
     ...
     */
    @Override
    public void updateConfig(InfraConfigPO config) {
        ...
        // 2. 重新查出最新数据更新本地缓存
        InfraConfigPO newestConfig = infraConfigMapper.selectById(config.getId());
